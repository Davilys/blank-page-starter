...
BEGIN
  SELECT jobid INTO existing_jobid FROM cron.job WHERE jobname = 'process-botconversa-contract-async';
  IF existing_jobid IS NOT NULL THEN PERFORM cron.unschedule(existing_jobid); END IF;
  PERFORM cron.schedule(
    'process-botconversa-contract-async', '* * * * *',
    $cron$ SELECT net.http_post(
      url := 'https://scpbqsvwojhbxihyqbdz.supabase.co/functions/v1/process-botconversa-contract-async',

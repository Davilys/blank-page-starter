...
DO $$ DECLARE jid bigint; BEGIN SELECT jobid INTO jid FROM cron.job WHERE jobname='process-webmarcas-continuous-agent'; IF jid IS NOT NULL THEN PERFORM cron.unschedule(jid); END IF;
 PERFORM cron.schedule('process-webmarcas-continuous-agent','* * * * *',$cron$ SELECT net.http_post(url:='https://scpbqsvwojhbxihyqbdz.supabase.co/functions/v1/process-webmarcas-continuous-agent',headers:='{"Content-Type":"application/json"}'::jsonb,body:='{}'::jsonb,timeout_milliseconds:=1000); $cron$); END $$;

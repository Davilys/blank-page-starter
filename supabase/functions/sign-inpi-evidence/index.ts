...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const accessClient = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_ANON_KEY')!, { global: { headers: { Authorization: req.headers.get('Authorization') || '' } } });
    const denied = await inpiAdminAccess(accessClient, req.headers.get('Authorization'), repair === true);

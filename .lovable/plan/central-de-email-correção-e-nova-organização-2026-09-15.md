...
- [caroline@webmarcas.net](mailto:caroline@webmarcas.net) — "falha de login", 19 erros seguidos; última mensagem gravada em 3/set.
- [juridico@webmarcas.net](mailto:juridico@webmarcas.net) — "falha de login", 29 erros seguidos; última mensagem gravada em 4/set.
- [financeiro@webmarcas.net](mailto:financeiro@webmarcas.net) — conexão derrubada pelo servidor, 17 erros seguidos; última entrada em 14/set.
- [ola@webmarcas.net](mailto:ola@webmarcas.net) — sincronizando normalmente (última entrada hoje).
...
- Nada será publicado em produção e nenhum reparo amplo será executado antes de eu apresentar o resultado dos testes e a amostra para aprovação. O plano está bem encaminhado, mas **corrija estes pontos antes de aprovar a execução**. Cole no Lovable:
  > Aprovo o escopo e a proposta visual, com os seguintes ajustes obrigatórios:
  >
  > **1. Diagnóstico por conta:** não concluir que as três contas têm senha incorreta. Caroline e Jurídico apresentam falha de autenticação; Financeiro apresenta conexão interrompida, que exige investigação própria. Verificar configuração, limites e resposta do servidor antes de pedir troca de senha.
  >
  > **2. Recebimento:** o sucesso da tarefa agendada não comprova sucesso na sincronização de cada conta. Medir e apresentar resultados por conta, incluindo mensagens efetivamente processadas e falhas.
  >
  > **3. Resposta automática:** minha escolha foi **“Manter, mas poder desligar por conta”**. Criar esse ajuste nas configurações, preservando o estado atual das contas. Impedir respostas duplicadas, loops entre robôs e disparos durante importação ou reparo de mensagens antigas.
  >
  > **4. Recuperação:** não classificar as 7.255 mensagens sem referência como definitivamente irrecuperáveis. Após restabelecer o acesso, verificar se existem no servidor ou em outra fonte disponível. Associar somente quando houver identificação confiável, sem correspondência apenas pelo assunto. Informar separadamente: recuperadas, não localizadas e recuperação inconclusiva.
  >
  > **5. Reparo piloto:** preservar os dados originais das 30 mensagens para reversão e apresentar antes/depois. Não sobrescrever conteúdo válido nem disparar automações.
  >
  > **6. Reversão:** restaurar inicialmente os componentes e funções anteriores, mantendo os dados e estruturas adicionados. Não excluir automaticamente tabelas ou colunas, pois podem conter histórico e mensagens recebidas após a atualização.
  >
  > Com esses ajustes, prossiga na versão de teste, exclusivamente dentro do módulo **Emails**, preservando o menu principal, os demais módulos e todas as funções de IA. Apresente os testes e a amostra antes da publicação.
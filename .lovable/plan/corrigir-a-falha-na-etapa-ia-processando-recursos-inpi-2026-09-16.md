...
- Escopo preservado: nenhuma outra modalidade, módulo ou integração é alterada; nada é publicado, protocolado ou enviado. Execute o plano com estes ajustes obrigatórios:
  1. **Streaming não garante conclusão.** Verifique também o limite total de execução da Edge Function. Se a geração puder ultrapassá-lo, use tarefa persistida com processamento em background e consultas curtas de andamento. Recarregar a página deve recuperar a tarefa existente.
  2. **Não concatene a continuação cegamente.** Preserve o texto parcial, permita apenas uma continuação e confira duplicações, seções ausentes, pedidos e encerramento antes de liberar a revisão. Se permanecer incompleto, salve como rascunho com erro recuperável. Remova a exigência artificial de 3.400 palavras: priorize fundamentação completa, pertinente e sem repetição.
  3. **Proteja a busca por** `caseId`**.** Antes de usar a service role, valide a sessão, a permissão e o acesso ao caso e aos documentos. Utilize os anexos da versão confirmada; não aceite caminhos arbitrários enviados pelo navegador.
  4. **Teste com autenticação legítima.** Não desative controles de acesso para chamar a função diretamente. Comprove as três modalidades chegando completas à revisão; teste também recarregamento, envio interrompido e continuação por truncamento. Se a interface não for testada, registre essa pendência e não declare homologação completa.
  Pode usar raciocínio médio e orçamento de 32.000 tokens, desde que suportados pelo modelo, mantendo a configuração restrita às três modalidades. Registre consumo e resultado para verificar o efeito.
  **Conclua essa correção em preview e apresente um relatório curto dos testes. Não acrescente funcionalidades nem inicie outra rodada de melhorias.**
...
import { AlertTriangle, MessageCircle, Mail, Loader2, RefreshCw, Search, CheckCircle2, Calendar } from "lucide-react";
...
export function OverdueChargeDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const overdue = (data || []).filter((i: any) => {
        ...
        // overdue if status is overdue OR pending+past due date
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <Tabs defaultValue="vencidos" className="flex-1 flex flex-col overflow-hidden">
          <TabsList>
          ...
          <TabsContent value="vencidos" className="flex-1 overflow-hidden flex flex-col gap-3">
            {/* Toolbar */}

...
export class AdminErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    // If it's an auth-session error, trigger global recovery (clean redirect to login)
    if (isAuthSessionError(error)) {
      forceAuthRecovery('ErrorBoundary: ' + error.message);
    }
  }
  ...
  render() {
    if (this.state.hasError) {
      const authExpired = isAuthSessionError(this.state.error);
          ...
          <div className="flex-1 flex items-center justify-center p-8">
            <div className="flex flex-col items-center gap-4 max-w-md text-center">
              ...
              <Button
                variant="default"
                size="sm"
                onClick={() => forceAuthRecovery('user clicked re-login')}
                className="gap-2"
              >

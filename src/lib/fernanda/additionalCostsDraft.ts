...
export const ADDITIONAL_COSTS_DRAFT = Object.freeze({
  status: 'blocked_pending_approval' as const,
  ...
  blockers: [
    'boleto_398_conflicts_with_approved_399',
    'diario_oficial_requires_caroline_rpi_validation',
  ],
});
...
export function detectsAdditionalCostIntent(message: string) {
  return /(?:algo|mais|extra|adicional|outra).*(?:pagar|pagamento|custo|taxa|valor)|(?:taxa|custo|valor).*(?:inpi|extra|adicional|depois)|(?:quanto).*(?:total|final)/i.test(message);
}
